...
void rtl8139_handler(registers_t * reg)
{
    ...
    if(status & RER)
    {
        ldprintf("RTL8139", LOG_WARN, "Error occurred when receiving packet");
    }

    if(status & TER)
    {
        ldprintf("RTL8139", LOG_ERR, "Error occurred when transmitting packet");
    }

    if(status & TOK)
    {
        ldprintf("RTL8139", LOG_DEBUG, "Packet sent\n");
    }
...
void init_rtl8139()
{
    ...
    {
        ldprintf("RTL8139", LOG_ERR, "Device not found");
        return;
    }
    if(isRTL8139Init == 0)
    {
        ...
        ldprintf("RTL8139", LOG_DEBUG, "use %s access (base: %x)", (rtl8139_dev.bar_type == 0)? "mem based":"port based", (rtl8139_dev.bar_type != 0)?rtl8139_dev.io_base:rtl8139_dev.mem_base);
        ...
        ldprintf("RTL8139", LOG_DEBUG, " Registered irq interrupt for rtl8139, irq num = %d", irq_num);
        ...
        ldprintf("RTL8139", LOG_INFO, "Initialized");
